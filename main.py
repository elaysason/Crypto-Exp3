...
def exp3_base(date, days, eta):
    start_date = datetime.datetime.strptime(date, "%Y-%m-%d")
    existing_coins = get_existing_coins(start_date, 1)
    coins_election = []
    scores = [0] * K
    reward = lambda choice, t: payoff(coins_list[choice], start_date, t, choice)
    best_sum = 0
    reward_sum = 0
    for t in range(1, days):
        ...
        cur_reward = payoff(coins_list[chosen_coin],start_date,t-1)
        for i in range(K):
            if i in existing_coins:
                scores[i] = scores[i] + 1
                if chosen_coin == i:
...
if __name__ == '__main__':
    ...
    regrets = [exp3_base("2018-01-01", t, eta) for t in days]
    print(regrets)
    x = range(100,1000,50)
